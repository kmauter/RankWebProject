...
def forgot_password():
    """Generate a new random password and return it to the user."""
    data = request.get_json()
    identifier = (data.get('identifier') or '').strip()

    if not identifier:
        return jsonify({'error': 'Please provide your username or email address.'}), 400
...
def create_game():
    try:
        ...
        max_submissions_per_user = int(data.get('maxSubmissionsPerUser') or 2)
...
def submit_song_to_game():
    try:
        ...
        if user_song_count >= int(game.max_submissions_per_user):
